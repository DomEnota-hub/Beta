...
def fresh(tag='tree'):
    global seq; seq+=1
    remote=f'/sdcard/stage4-{seq}.xml'; adb('shell','uiautomator','dump',remote)
    data=adb('shell','cat',remote); (OUT/f'{seq:03d}-{tag}.xml').write_bytes(data)
...
def tap_visible(text,contains=False,allow_focus=False):
    ...
    n=tappable(root,ms[0],allow_focus); b=bnd(n); x=(b[0]+b[2])//2; y=(b[1]+b[3])//2
    adb('shell','input','tap',str(x),str(y)); time.sleep(.8); return True
...
def clickable(text):
    ...
    ps=pmap(root); n=ms[0]
    while n in ps:
        if n.get('clickable')=='true': return True
        n=ps[n]
    return n.get('clickable')=='true'
...
def open_acceptance():
    ...
    expect_visible('Полная приёмка',timeout=45)

def select_state(name): tap_scroll(name,max_swipes=28)

def expect_step(n,total=66):
    scroll_top(); expect_visible(f'Шаг {n} из {total}',timeout=30,contains=True)
...
try:
    ...
    expect_visible('Железнодорожный помощник',timeout=90)
    ...
    if clickable('Далее'): raise AssertionError('Next is clickable before BLOCKING justification')
    ...
    if clickable('Далее'): raise AssertionError('Next is clickable before NOT_APPLICABLE reason')
    ...
    open_acceptance(); tap_scroll('Начать из кабины'); expect_visible('Пошаговая приёмка',timeout=30)
    ...
    if clickable('Далее'): raise AssertionError('Next is clickable while safety gate is unconfirmed')
