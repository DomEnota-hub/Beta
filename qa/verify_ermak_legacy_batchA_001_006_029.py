...
for sid in TARGETS:
    s=by[sid]
    user_text=' '.join(
        [n.get('prompt','')+' '+n.get('text','')+' '+n.get('result','') for n in s['graph']['nodes']] +
        [a.get('summary','') for a in s.get('actions',[])]
    ).lower()
    assert 'нажав рукой' not in user_text, sid
    assert 'перевести тормозные переключатели qt1 в режим «тяга» вручную' not in user_text, sid
    assert 'поочередно включая секции определить' not in user_text, sid
